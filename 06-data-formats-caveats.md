...
In the [previous lesson](05-exporting-data.md) we discussed how to export Excel file formats into `*.csv`. Comma Separated Value files are indeed very useful allowing for easily exchanging and sharing data. A similarly useful option is `*.tsv`, Tab-Separated Values.

However, there are some significant problems with these particular formats. Quite often the data values themselves may include commas (,) or tabs. In that case, the software which you use (including Excel) will most likely incorrectly display the data in columns. It is because the commas which are a part of the data values will be interpreted as a delimiter.
